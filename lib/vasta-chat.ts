...
export function watchTyping(conversationId: string, onChange: (uid: string | null) => void, currentUid: string): Unsubscribe {
  return onSnapshot(collection(db, "conversations", conversationId, "typing"), (snapshot) => {
    const active = snapshot.docs.map((item) => ({ uid: item.id, ...(item.data() as { active?: boolean }) })).find((item) => item.uid !== currentUid && item.active);
...
export async function findProfileByPhone(phone: string): Promise<VastaProfile | null> {
  ...
  const indexSnapshot = await getDoc(doc(db, "phoneIndex", phoneIndexId(normalized))); if (!indexSnapshot.exists()) return null;
  const { uid } = indexSnapshot.data() as { uid: string }; const profileSnapshot = await getDoc(doc(db, "users", uid));
...
export async function openConversation(current: VastaProfile, other: VastaProfile) {
  if (current.uid === other.uid) return null; const id = conversationIdFor(current.uid, other.uid);
  const payload: Omit<VastaConversation, "id"> = { participants: [current.uid, other.uid], names: { [current.uid]: current.displayName, [other.uid]: other.displayName }, lastMessage: "ابدأ المحادثة ✨", lastMessageAt: Date.now() };
  await setDoc(doc(db, "conversations", id), payload, { merge: true }); return { id, ...payload };
}

export async function sendTextMessage(conversationId: string, sender: VastaProfile, text: string, reply?: VastaMessage) {
  const value = text.trim(); if (!value) return; const now = Date.now();
  await addDoc(collection(db, "conversations", conversationId, "messages"), { kind: "text", text: value, senderId: sender.uid, createdAt: now, ...(reply ? { replyToId: reply.id, replyToText: reply.deleted ? "رسالة محذوفة" : reply.text } : {}) });
  await setDoc(doc(db, "conversations", conversationId), { lastMessage: value, lastMessageAt: now }, { merge: true });
}

export async function sendVoiceMessage(conversationId: string, sender: VastaProfile, audioUrl: string, storagePath: string, durationMs: number, mimeType: string) {
  const now = Date.now(); await addDoc(collection(db, "conversations", conversationId, "messages"), { kind: "voice", text: "", senderId: sender.uid, createdAt: now, audioUrl, storagePath, durationMs, mimeType });
  await setDoc(doc(db, "conversations", conversationId), { lastMessage: "🎤 رسالة صوتية", lastMessageAt: now }, { merge: true });
}

export async function sendMediaMessage(conversationId: string, sender: VastaProfile, media: { mediaUrl: string; storagePath: string; mediaKind: "image" | "video" | "file"; fileName: string; sizeBytes: number; mimeType: string }, reply?: VastaMessage) {
  const now = Date.now(); await addDoc(collection(db, "conversations", conversationId, "messages"), { kind: "media", text: "", senderId: sender.uid, createdAt: now, ...media, ...(reply ? { replyToId: reply.id, replyToText: reply.deleted ? "رسالة محذوفة" : reply.text } : {}) });
  await setDoc(doc(db, "conversations", conversationId), { lastMessage: media.mediaKind === "image" ? "🖼️ صورة" : media.mediaKind === "video" ? "🎬 فيديو" : "📄 ملف", lastMessageAt: now }, { merge: true });
}

export async function editOwnMessage(conversationId: string, messageId: string, text: string) {
  const value = text.trim(); if (!value) return; await updateDoc(doc(db, "conversations", conversationId, "messages", messageId), { text: value, edited: true, editedAt: serverTimestamp() });
}
export async function deleteOwnMessage(conversationId: string, messageId: string) {
  await updateDoc(doc(db, "conversations", conversationId, "messages", messageId), { text: "", deleted: true, edited: false, deletedAt: serverTimestamp() });
}
...
export async function setMessageReaction(conversationId: string, messageId: string, uid: string, emoji: string) {
  const ref = doc(db, "conversations", conversationId, "messages", messageId, "reactions", uid);
  if (!emoji) return deleteDoc(ref); return setDoc(ref, { emoji, updatedAt: serverTimestamp() });
}
export function watchMessageReactions(conversationId: string, messageId: string, onChange: (reactions: Record<string, string>) => void): Unsubscribe {
  return onSnapshot(collection(db, "conversations", conversationId, "messages", messageId, "reactions"), (snapshot) => {
    const next: Record<string, string> = {}; snapshot.forEach((item) => { const data = item.data() as { emoji?: string }; if (data.emoji) next[item.id] = data.emoji; }); onChange(next);
  });
}
