...
export async function findProfileByPhone(phone: string): Promise<VastaProfile | null> { const normalized = normalizePhone(phone); if (!normalized) return null; const indexSnapshot = await getDoc(doc(db, "phoneIndex", phoneIndexId(normalized))); if (!indexSnapshot.exists()) return null; const { uid } = indexSnapshot.data() as { uid: string }; const profileSnapshot = await getDoc(doc(db, "users", uid)); if (!profileSnapshot.exists()) return null; const profile = profileSnapshot.data() as VastaProfile; return { ...profile, phoneNumber: "" }; }
export async function findProfileByUsername(username: string): Promise<VastaProfile | null> { const normalized = normalizeUsername(username); if (!normalized) return null; const indexSnapshot = await getDoc(doc(db, "usernameIndex", usernameIndexId(normalized))); if (!indexSnapshot.exists() || indexSnapshot.data().retired) return null; const { uid } = indexSnapshot.data() as { uid: string }; const profileSnapshot = await getDoc(doc(db, "users", uid)); if (!profileSnapshot.exists()) return null; const profile = profileSnapshot.data() as VastaProfile; return { ...profile, phoneNumber: "" }; }
